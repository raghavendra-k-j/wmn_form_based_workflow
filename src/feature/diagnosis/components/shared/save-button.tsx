...
export const SaveButton = observer(() => {
  ...
  const isDisabled = store.currentDiagnoses.length === 0 || store.saveState === 'saving';
  ...
  return (
    ...
    >
      ...
      {store.saveState === 'saving' ? 'Saving...' : store.saveState === 'saved' ? 'Saved' : store.saveState === 'error' ? 'Error' : 'Save'}
    </button>
  );
});
