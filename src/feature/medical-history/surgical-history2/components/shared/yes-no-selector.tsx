...
export const YesNoSelector = observer(() => {
  ...
  return (
    <div className="flex items-center gap-4">
      <span className="text-[13px] font-medium text-gray-700">Does the patient have surgical history?</span>
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => store.setHasSurgicalHistory(true)}
          className={`
            px-4 py-1.5 text-[12px] font-medium border transition-colors
            ${store.hasSurgicalHistory === true
        ...
        <button
          type="button"
          onClick={() => store.setHasSurgicalHistory(false)}
          className={`
            px-4 py-1.5 text-[12px] font-medium border transition-colors
